...
def build_queries(date: datetime.date) -> list:
    ...
    ds = str(date)
    return [
        f"한국 IT 이커머스 스타트업 권고사직 희망퇴직 구조조정 대표이사 CTO {ym}",
        f"고용노동부 채용 HR 노무 트렌드 지원사업 {ym}",
        f"글로벌 빅테크 외국계 기업 감원 채용 한국 {ym}",
        f"한국 채용 기업 인사 뉴스 {ds}",
        f"Korea hiring layoffs HR news {ds}",
    ]
...
【분량】최소 {MIN}개 items. 동일 사건 기사 → 1개 통합, sources에 모두 열거.
